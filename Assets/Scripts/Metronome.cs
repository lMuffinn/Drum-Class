    ...
    void NextBeat()
    {
        if (gameLeader)
        {
            if (timeFromStart >= StartDelay)
            {
                ...
                imageClone.GetComponent<Transform>().position = GetComponent<Transform>().position;
                Debug.Log("made beat");
            }
        }
        if (!gameLeader)
        {
            if (timeFromStart >= StartDelay)
            {
                ...
                imageClone.GetComponent<ArrowMovement>().totalTime = timeBeforeHit;
                imageClone.GetComponent<ArrowMovement>().empty = beatQueue.Peek().gameObject.GetComponent<ArrowMovement>().empty;
                imageClone.GetComponent<AudioSource>().clip = beatQueue.Peek().sound;
                imageClone.GetComponent<Transform>().position = GetComponent<Transform>().position;
                Debug.Log("made dumbeat");
            }
        }
    }
}
